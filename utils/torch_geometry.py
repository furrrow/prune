...
"""
torch_geometry.py
borrowed from the hc_net paper codebase, see
https://github.com/xlwangDev/HC-Net/blob/main/models/utils/torch_geometry.py
"""
...
def hard_code_perspective(yaw_deg, out_size):
    """Calculates perspective H2 according to paper's appendix
    """
    ...
    h_2[0][0] = np.cos(yaw)
    h_2[0][1] = -np.sin(yaw)
    h_2[0][2] = u_c * (1 - np.cos(yaw)) + v_c*np.sin(yaw)
    h_2[1][0] = np.sin(yaw)
    h_2[1][1] = np.cos(yaw)
    h_2[1][2] = v_c * (1 - np.cos(yaw)) - u_c * np.sin(yaw)
    h_2[2][0] = 0
    h_2[2][1] = 0
    h_2[2][2] = 1
    return h_2  # 3x3
